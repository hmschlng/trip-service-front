  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        ...
        // extractResponseData 함수 사용하여 데이터 추출
        ...
        if (Array.isArray(responseData)) {
          ...
          setReviews(responseData.content); // 페이지네이션 응답인 경우

        } else {
          setReviews([]);// 기타 경우에는 빈 배열 설정

        }
      } catch (error: any) {
        console.error('Error fetching reviews:', error);
        setError(error.response?.data?.message || '여행 후기를 불러오는데 실패했습니다.');
